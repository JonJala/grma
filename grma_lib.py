...
# -------------------------
def rel_preprocess(rel_input: Union[str, pd.DataFrame], id_filename: str, rel_degree: str, sample_indices_to_keep: List[int]) -> tuple[THRESHOLDED_REL_TYPE, np.ndarray]:
    MAX_RELATEDNESS = 4  # Maximum degree of relatedness from king output
    
    
    # Read in King output and filter out unneeded rows (where relatedness is too weak)
    if isinstance(rel_input, str):
        king_df = pd.read_csv(rel_input, sep=r"\s+")[NEEDED_KING_COLS]
    elif isinstance(rel_input, pd.DataFrame):
        king_df = rel_input
    else:
        raise TypeError(f"Type of parameter rel_input ({type(rel_input)}) is not supported.")
    ...
    # Use the .fam file to get FID/IID mapping to person number
    id_df = _get_id_df_from_fam_file(id_filename, sample_indices_to_keep)
    N = len(id_df)
    ...
    logging.info(f"Len of rel_info is {len(rel_info)}")
